#!/usr/bin/env bash
# Build the Matou Android *release* APK for direct install / COA builds
# (matou-app #245, Matou/coa ADR 0004: COA communities sideload the APK).
#
#   scripts/android/build-release-apk.sh [--allow-unsigned]
#
# Copy of build-aab.sh with bundleRelease -> assembleRelease (kit plan Task 9):
# same preparation (bakes the config-server URL into the MatouBackend plugin
# config, makes sure the gomobile .aar exists), same signing/env checks, then
# the Quasar Capacitor build in release mode and `gradlew assembleRelease`.
# Output:
#   frontend/dist/capacitor/android/apk/release/app-release.apk
#
...
#   * refuses to produce an UNSIGNED APK unless --allow-unsigned is given
#     (Android refuses to install unsigned APKs; useful only for smoke-testing
#     the build pipeline).
#
...
if [ "$have_signing" = 0 ]; then
  if [ "$ALLOW_UNSIGNED" = 1 ]; then
    echo "WARN: no signing config — building an UNSIGNED APK (--allow-unsigned)" >&2
  else
...
# Same shape as build-aab.sh (build the UI with --skip-pkg, cap sync, run the
# gradle target ourselves) so both release outputs go through one code path
# and the same signing config.
cd "$FRONTEND"
...
if [ ! -f "$APK" ]; then
  # An unsigned assembleRelease names the artefact app-release-unsigned.apk.
  APK="$ANDROID/app/build/outputs/apk/release/app-release-unsigned.apk"
fi
OUT_DIR="$FRONTEND/dist/capacitor/android/apk/release"
if [ ! -f "$APK" ]; then
  echo "ERROR: expected APK not found under $ANDROID/app/build/outputs/apk/release/" >&2
  exit 1
fi
mkdir -p "$OUT_DIR"
cp "$APK" "$OUT_DIR/app-release.apk"
...
if command -v jarsigner >/dev/null 2>&1 && [ "$have_signing" = 1 ]; then
  echo "==> signer:"
  jarsigner -verify -verbose:summary -certs "$OUT_DIR/app-release.apk" 2>/dev/null \
