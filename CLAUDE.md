...
| `MATOU_KERIA_KEYSTATE_URL` | Key-state URL template(s) with `{aid}` (default `{cesrUrl}/oobi/{aid}`); plain http only to loopback unless `MATOU_KERIA_KEYSTATE_ALLOW_HTTP=1`. Accepts a **comma-separated ordered list** of templates tried in turn until one serves the KEL — list a witness `/oobi/{aid}` after the KERIA OOBI so a group AID a co-signer's agent 404s, or an AID mid-receipting, still resolves (each source is held to the same loopback/TLS rule). A transient 404/503 (the witness-receipting window right after a rotation) is retried with bounded backoff (~2.4s budget) before the verifier degrades a request to unauthenticated, rather than failing on the first attempt. See #513. |
