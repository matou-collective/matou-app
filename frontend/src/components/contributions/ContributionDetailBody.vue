<template>
  <q-card class="detail-dialog" :class="{ 'detail-dialog--inline': inline }">
      <!-- Sticky header -->
      <div class="dialog-sticky-header">
        ...
        <h2 class="header-title">
          ...
          <q-btn
            ...
            @click="emit('edit-contribution', contribution)"
          >
          ...
          <q-btn
            ...
            icon="rule"
            color="primary"
            class="title-change-btn"
            @click="showChangeDialog = true"
          >
            <q-tooltip>Request changes</q-tooltip>
          </q-btn>
        </h2>
        <q-btn
          v-if="!inline"
      ...
      <div class="dialog-body">
        ...
        <div class="stats-grid">
          <div class="stat-card">
            ...
            <div class="stat-value">{{ contribution.estimated_duration ? `${contribution.estimated_duration}h` : '—' }}</div>
          </div>
          <div v-if="contribution.actual_duration" class="stat-card">
          ...
          <div class="stat-card">
            ...
            <div class="stat-value">{{ contribution.deadline ? formatDate(contribution.deadline) : '—' }}</div>
          </div>
        </div>
        ...
        <div class="content-section">
          <h3 class="section-title row items-center q-gutter-sm">
            ...
            <span>Discussion ({{ contributionComments.length }})</span>
          </h3>
          <div v-if="contributionComments.length === 0" class="empty-discussion">
            No comments yet. Be the first to share your thoughts!
          </div>
          <div v-else class="comments-list">
            <div
              v-for="c in contributionComments"
              :key="c.id"
              class="comment-row"
...
interface Props {
  ...
  canArchive?: boolean;
  /** When true, the body renders inline (close button hidden, no q-dialog wrapper). */
  inline?: boolean;
}
...
watch(() => props.contribution.id, (id) => {
  ...
  localChildren.value = [];
  if (id) {
    void store.fetchComments(id).then(() => {
      const count = store.commentsByContribution[id]?.length ?? 0;
      void commentCursorsStore.markRead('contribution', id, count);
    });
  }
}, { immediate: false });

// --- Comments ---
const contributionComments = computed(
  () => store.commentsByContribution[props.contribution.id] ?? [],
);
...
watch(lastEvent, (event) => {
  if (!event) return;
  if (event.type !== 'contribution:comment_added') return;
  const data = event.data as { contribution_id?: string } | undefined;
  if (data?.contribution_id === props.contribution.id) {
    void store.fetchComments(props.contribution.id).then(() => {
...
const canEditContribution = computed(() => {
  // Leads/stewards can edit, except after sign-off / reward / archive.
  if (!(isLead.value || isSteward.value)) return false;
  return !['signed_off', 'rewarded', 'archived'].includes(props.contribution.status);
});
...
async function handleChange(data: { updates: Record<string, unknown>; reason: string }) {
  ...
  try {
    ...
    // Project lead and assigned-contributor edits also require re-confirmation.
    const assigneeChanged = !!data.updates.assigned_contributor_id;
    const assignedId =
      props.contribution.assigned_contributor_id ?? props.contribution.assigned_contributor ?? null;
    const editedByAssignee = !!props.currentUserId && assignedId === props.currentUserId;
    const needsReconfirm =
      role.value === 'project_lead' || assigneeChanged || editedByAssignee;
    if (needsReconfirm) {
      const transitioned = await store.transition(props.contribution.id, 'changed');
...
// Inline mode (used on the standalone contribution detail page): drop the
// modal max-height so the page scrolls naturally.
.detail-dialog.detail-dialog--inline {
  max-height: none;
  margin: 0 auto;
