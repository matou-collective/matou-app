...
describe('electron-builder config from kit.build.json', () => {
  it('maps the default kit to upstream values', () => {
    ...
    expect(c.productName).toBe('Matou');
    expect(c.artifactName).toBe('matou-${version}-${platform}.${ext}');
    expect(c.mac.artifactName).toBe('matou-${version}-${platform}-${arch}.${ext}');
  ...
  it('maps a community kit to coa values and publish null', () => {
    ...
    expect(c.artifactName).toBe('x-y-${version}-${platform}.${ext}');
  });
});
