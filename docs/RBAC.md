...
**¹ Proof-backed (GH#19 part 2 — meets AC-1).** The change must carry a KERI **action proof** (`sign_off_proof` / `reward_proof` / `proof` field) — a Cigar signature over the canonical message `matou-proof/v1\n<action>\n<subject>\n<space>\n<value>\n<dt>` (single source of truth: `frontend/src/lib/keri/actionProof.ts`). The verifier (`proof_verifier.go`) rebuilds that message from the object's **own** authoritative fields (never a copy on the object) and verifies the signature against the signer AID's KEL signing key using the CESR + ed25519 primitives from GH#18 (`internal/auth`). The signer AID is thus authenticated cryptographically, so the attacker-controlled ACL-join-metadata binding is no longer trusted: a writer-permission forger cannot mint a valid signature, cannot lift a real steward's proof onto a different object (subject binding) or space (space binding). When the proof carries a signing-time KEL sequence number (`sn`), the signature is verified against the key state **as of that sn** so a later legitimate rotation does not invalidate it (part 3 / #112, see below); otherwise it is checked against the current key (fail-closed on rotation). The verified AID's role is then resolved by AID (`RolesForAIDAt`) as of the change's timestamp, and — when the credential/TEL seam is wired — an unrevoked org credential granting a permitted role is additionally required (part 3 / #112). Enforcement is gated by `MATOU_REQUIRE_SIGNED_AUTH` (same signal that makes `X-User-AID` trustworthy); when off (dev/test without live KERIA) proof-backed rules fall back to the role-only path so a missing proof never breaks a legitimate action.
...
**KEL-sn anchoring (GH#19 part 3 / #112 — resolved).** The proof envelope now carries the signer's KEL sequence number at signing time (`sn`, emitted from `hab.state.s` in `frontend/src/lib/keri/signProof.ts`; `contributions.Proof.SN`). When present, the verifier resolves the signing key **as of that sn** — the most recent establishment event with sequence ≤ `sn`, via `auth.ExtractKeyStateAt` / `StaticKeyProvider.SigningKeysAt` (the `AnchoredKeyProvider` seam) — so a proof stays valid after a **later legitimate rotation** past `sn`, instead of failing closed. `sn` is **not** part of the signed canonical message (no `PROOF_VERSION` bump): it only selects which historical key state to check against, and a tampered `sn` resolves a key the signature will not match. Proofs without an `sn` (older wire format) still verify against the current key (fail-closed on rotation). The write-rule refresher snapshots each member AID's establishment history when the key-state resolver implements `auth.KeyHistoryResolver` (`KERIAResolver.KeyHistory`); absent history, `SigningKeysAt` falls back to current keys.

**Credential / TEL role binding (GH#19 part 3 / #112 — mechanism resolved, live population deferred).** A proof-backed transition can now additionally require the crypto-verified signer AID to hold a **valid, unrevoked org credential** granting a role the transition permits, via the `CredentialVerifier` seam (`anysync.WriteRuleValidator.WithCredentialVerifier`, `SnapshotCredentialVerifier`). A credential revoked **as of the change timestamp** contributes no role, so a **revoked credential is rejected even when the synced `CommunityProfile` role history still records the role** (a lagging profile cannot rescue it). This is enforced by fixture tests (`credential_verifier_test.go`, `write_rules_test.go:TestValidateChange_RevokedCredentialRejected`). It is **not yet wired in `internal/app`**: there is no synced representation of credential TEL/revocation status in-repo (revocation happens client-side via KERIA), so a live snapshot cannot yet distinguish revoked from active credentials, and attaching an empty verifier would fail-closed on legitimate transitions. The check therefore stays opt-in until the TEL snapshot source lands (a synced credential-status / TEL tree — the remaining work).

**Remaining follow-ups (do not block AC-1):**

- Populating the sn-anchored key-state history and the credential/TEL snapshot from live KERIA/synced state needs a reachable KERIA endpoint (and, for credentials, a synced TEL/revocation representation that does not yet exist); verified by the e2e run (NEEDS LIVE VERIFICATION), not in-sandbox unit tests. The seams and fixture tests are in place.
