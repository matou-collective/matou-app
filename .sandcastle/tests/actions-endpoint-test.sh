...
# 1. No script reads the runs listing. Comment lines are exempt (this hazard
#    has to be explainable in prose next to the code it constrains); anything
#    executable naming it is the regression this ticket exists to prevent.
#    Note the web link `<server>/<repo>/actions/runs/<n>` in a workflow's
#    RUN_URL is a browser URL, not an API read, and lives in .yml — out of
#    this scan by construction.
runs_readers="$(grep -n 'actions/runs' $shells 2>/dev/null | grep -v ':[0-9]*:[[:space:]]*#' || true)"
check "no harness script reads /actions/runs" '[ -z "$runs_readers" ] || { printf "%s\n" "$runs_readers"; false; }'
...
fetchers="$(grep -n 'actions/tasks' $shells 2>/dev/null | grep -v ':[0-9]*:[[:space:]]*#' | cut -d: -f1 | sort -u)"
untimed=""
