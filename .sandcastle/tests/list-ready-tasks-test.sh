#!/usr/bin/env bash
# Offline test for the swarm task source: the standing #183 rehearsal DRIVE
# issue (REHEARSAL_DRIVE_ISSUE, default 492) is executed host-mode by the
# workstation cron scripts/rehearsal-executor.sh — Ben's ruling on #380
# (DO/broker secrets stay off the swarm containers, #377 proved the substrate
# cannot drive). So list-ready-tasks.sh must NEVER surface it to the swarm,
# even though it is open + ready-for-agent + unblocked — otherwise a swarm
# iteration hot-loops on an issue it structurally cannot run. Every OTHER
# rehearsal-183 issue (the holes the swarm plugs) must still come through.
# Drives the REAL script with a shimmed curl. Run:
...
mkdir -p "$tmp/bin"
# Pin REHEARSAL_DRIVE_ISSUE to the fixture's drive number (#586). list-ready-tasks.sh
# defaults it to 492 but the host's .sandcastle/.env re-mints a different drive
# per run (#493) and leaks it into this suite's ambient env; without this pin,
# case 1 asserts #492 is excluded as the drive while the script reads the ambient
# value (523 today) and surfaces #492 — the same hardcoded-fixture-vs-ambient-env
# shape #537 pinned in rehearsal-healer-test.sh. Case 4 still overrides inline.
export PATH="$tmp/bin:$PATH" FORGEJO_TOKEN=t REHEARSAL_DRIVE_ISSUE=492
...
# Fixture: the standing drive issue #492 (number backstop, deliberately
# unlabelled), a future re-minted drive #700 wearing the `standing-drive`
# label (the PRIMARY exclusion, #493 ruling), plus two ordinary ready swarm
# tasks, one of them a rehearsal-183 hole (#999) — that label alone must NOT
# exclude it.
...
out="$(bash "$here/../list-ready-tasks.sh")" || fail "script exited non-zero"

# 1: the standing drive issue is gone from the swarm queue
if jq -e '.[] | select(.number == 492)' <<<"$out" >/dev/null; then
  fail "the standing drive issue #492 must not be surfaced to the swarm"
fi
pass=$((pass+1))

# 1b: a `standing-drive`-labelled issue is excluded regardless of its number —
# the label is the primary exclusion; a re-mint needs no code/env edit
...
# with an empty arg and exit 123. This is the real live state today.)
cat > "$ISSUE_FIXTURE" <<'JSON'
[
  {"number": 492, "title": "rehearsal: THE DRIVE", "body": "standing drive issue", "html_url": "u/492"}
]
JSON
out3="$(bash "$here/../list-ready-tasks.sh")" || fail "empty-queue run must exit 0"
