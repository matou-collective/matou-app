...
#!/usr/bin/env bash
set -u
url="" data="" method=GET want_code=false prev=""
for a in "$@"; do
  case "$a" in
    -X) : ;;
    -w) want_code=true ;;
...
export CALLS_LOG="$tmp/calls.log" BODIES_LOG="$tmp/bodies.log"
touch "$CALLS_LOG" "$BODIES_LOG"
...
check "forgejo_get resolves relative to FORGEJO_API" \
  'grep -q "GET http://fj.test/api/v1/repos/Matou/ourcloud/labels?limit=100" "$CALLS_LOG"'
...
check "forgejo_comment posts to the issue's comments endpoint" \
  'grep -q "POST http://fj.test/api/v1/repos/Matou/ourcloud/issues/42/comments" "$CALLS_LOG"'
...
: > "$BODIES_LOG"
code="$(FAKE_CODE=201 forgejo_add_dependency 42 991 Matou ourcloud)"
check "forgejo_add_dependency returns the HTTP code" '[ "$code" = 201 ]'
check "forgejo_add_dependency body names index+owner+repo (#381)" \
  '[ "$(jq -c "{index,owner,repo}" < "$BODIES_LOG")" = "{\"index\":991,\"owner\":\"Matou\",\"repo\":\"ourcloud\"}" ]'
code="$(FAKE_CODE=409 forgejo_add_dependency 42 991 Matou ourcloud)"
...
check "forgejo_repo_slug derives owner/repo from FORGEJO_API" '[ "$(forgejo_repo_slug)" = Matou/ourcloud ]'
