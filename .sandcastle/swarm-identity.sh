...
# scripts (claim-next-task.sh, heal.sh, schedule-backstop.sh, ...) need a
# FORGEJO_API/WORKDIR default but must never hardcode one — a hardcoded
# "Matou/matou-app" default baked into a byte-identical file is exactly the
# "slug as value" half of harness-sync's ownership-inversion bug the
# extraction retires. Every per-repo default collapses into this ONE file
# instead of being copy-pasted into every script that needs it (8 copies at
# 2026-08-15 survey time).
#
